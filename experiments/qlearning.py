"""
Fixed & robust SUMO + TraCI RL script (Tabular Q-Learning).

Metrics added:
 - Logs queue length at every step
 - Computes total delay, avg queue, max queue, time to clear
 - Saves results to results_ql.npz for comparison plotting
"""
...
STEP_LENGTH = 0.10  # must match --step-length
...
def compute_time_metrics(queue_history, step_length=STEP_LENGTH):
    if not queue_history:
        return 0.0, 0.0, 0, None
    total_delay = sum(q * step_length for q in queue_history)  # veh-sec
    avg_queue = float(sum(queue_history)) / len(queue_history)
    max_queue = max(queue_history)
    time_to_clear = None
    for i, q in enumerate(queue_history):
        if q == 0:
            time_to_clear = i * step_length
            break
    return total_delay, avg_queue, max_queue, time_to_clear
...
try:
    ...
    print("\n=== Starting Fully Online Continuous Learning (Tabular Q-Learning) ===")
    for step in range(TOTAL_STEPS):
        ...
        # Log every step (for time analysis)
        step_history.append(step)
        reward_history.append(cumulative_reward)
        queue_history.append(sum(new_state[:-1]))

        # Print only every 100 steps to reduce spam
        if step % 100 == 0:
            print(f"Step {step}, State={state}, Action={action}, NewState={new_state}, "
...
if len(step_history) > 0:
    total_delay, avg_queue, max_queue, time_to_clear = compute_time_metrics(queue_history)
    print("\n=== Q-Learning Time Analysis ===")
    ...
    if time_to_clear is not None:
        print(f"Time until queue first clears: {time_to_clear:.1f} s")
    else:
        print("Queue never fully cleared during the simulation.")

    # Save for comparison script
    np.savez("results_ql.npz",
             step=np.array(step_history, dtype=np.int32),
...
if len(step_history) > 0:
    try:
        ...
        plt.plot([s * STEP_LENGTH for s in step_history], reward_history, marker='o', linestyle='-')
        ...
        plt.figure(figsize=(10, 5))
        plt.plot([s * STEP_LENGTH for s in step_history], queue_history, marker='o', linestyle='-')
        plt.xlabel("Simulation Time (s)")
